...
- Plain `←` exits the native session only when its composer is empty; otherwise
  it remains Codex's character-left key
...
- The Agents View PTY bridge conservatively tracks whether Codex's composer is
  empty. Plain left-arrow returns to the overview only when emptiness is known;
  ambiguous history/editor states remain inside Codex to avoid losing a draft.
